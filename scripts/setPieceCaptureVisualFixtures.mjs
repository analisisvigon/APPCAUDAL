...
const names = ['BURU', 'NIÑO', 'D. RUIZ', 'FASSANI', 'M. SECADES', 'TRABANCO', 'MADRIGAL', 'TINEO', 'XURDE', 'JAVI', 'ÁLVARO'];
const escapeHtml = (value) => String(value ?? '').replaceAll('&', '&amp;').replaceAll('<', '&lt;')
  .replaceAll('>', '&gt;').replaceAll('"', '&quot;');
const roleEntries = (roles) => Object.fromEntries(roles.flatMap((id, index) => id ? [[
  `rival-player-${index}`, { responsibilityId: id, positionKey: `rival:${index}` },
]] : []));
const xi = Object.fromEntries(Array.from({ length: 11 }, (_, index) => [`rival:${index}`, `rival-player-${index}`]));
...
const cases = {
  defensive_complete: {
    title: 'ABP defensiva del rival', action: 'Córner defensivo', type: 'defensive_set_piece',
    ball: { x: 5, y: 5 },
  ...
  offensive_complete: {
    title: 'ABP ofensiva del rival', action: 'Córner ofensivo', type: 'offensive_set_piece',
...
for (const [name, scenario] of Object.entries(cases)) {
  ...
  const rivalPositions = Array.from({ length: 11 }, (_, index) => positions[`rival:${index}`]);
  const rivalMarkers = rivalPositions.map((point, index) => {
    const playerId = currentXi[`rival:${index}`];
    ...
    const placement = getSetPieceBadgePlacement(index, rivalPositions, viewport);
