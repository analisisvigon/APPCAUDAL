...
assert.doesNotMatch(appSource, /playerPdfCrestAudit|COPIAR DIAGNÓSTICO PDF|buildPlayerPdfDiagnostic|qaCrestLoadAudit/, 'la instrumentación temporal del dossier se elimina por completo');
assert.match(appSource, /imageFallbackLoader:\s*\(imageUrl\) => loadAuthenticatedExternalImage\(supabase, imageUrl\)/, 'el PDF reutiliza el fallback server-side autenticado para imágenes bloqueadas por CORS');
