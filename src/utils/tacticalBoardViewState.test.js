...
assert.match(appSource, /isSetPieceCapture \? Boolean\(rivalSlot\.player\) : layers\.rivalNames/,
  'RIVAL controla el texto normal y la captura solo muestra nombres de jugadores resueltos');
assert.match(appSource, /\(layers\.caudalNames \|\| isSetPieceCapture\) \? <span/,
  'CAUDAL conserva su control de texto fuera de la captura ABP');
...
assert.match(appSource, /\{\(layers\.rival \|\| \(tacticalCaptureMode && tacticalGamePhase === 'set_piece'\)\) \? rivalSlots\.map/,
  'ocultar texto no altera la visibilidad normal y ABP captura fuerza los jugadores rivales');
assert.match(appSource, /\{layers\.caudal && !\(tacticalCaptureMode && tacticalGamePhase === 'set_piece'\) \? caudalCoordinates\.map/,
  'Caudal conserva su capa normal y desaparece únicamente en captura ABP');
