...
assert.match(cssSource, /\.team-presentation-bench-groups\s*\{[\s\S]*grid-template-columns: repeat\(auto-fit, minmax\(min\(100%, 280px\), 1fr\)\);/, 'las categorías aprovechan automáticamente el ancho sin estrechar prematuramente los nombres');
...
assert.match(numberNameSource, /getPlayerDisplayName\(player\)/, 'la identidad compartida prioriza nombre de camiseta');
...
assert.match(numberNameSource, /min-w-0 truncate font-black text-white/);
