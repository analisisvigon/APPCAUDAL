...
export function FinesHorizontalRanking({
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-white/[0.07] bg-black/15 p-3 sm:p-4">
      <h4 className="text-sm font-black text-white">{title}</h4>
      {subtitle ? <p className="mt-1 text-xs leading-4 text-slate-500">{subtitle}</p> : null}
      {!ranked.length ? <p className="mt-3 rounded-xl border border-dashed border-white/[0.08] px-3 py-3 text-center text-xs font-bold text-slate-500">{emptyText}</p> : null}
      {ranked.length ? (
        <ol className="mt-3 space-y-3" aria-label={title}>
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-white/[0.07] bg-black/15 p-3 sm:p-4">
      <div className="flex items-start justify-between gap-3">
        <div><h4 className="text-sm font-black text-white">Distribución por estados</h4><p className="mt-1 text-xs leading-4 text-slate-500">Las vencidas forman parte de las multas pendientes o parciales.</p></div>
        {overdue > 0 ? <span className="shrink-0 rounded-full border border-red-300/20 bg-red-400/10 px-2 py-1 text-[10px] font-black uppercase text-red-100">{overdue} vencidas</span> : null}
      </div>
      {total > 0 ? <div className="mt-4 flex h-4 overflow-hidden rounded-full bg-white/[0.06]" role="img" aria-label="Distribución visual de multas por estado">{segments.filter((segment) => segment.value > 0).map((segment) => <div key={segment.key} className={segment.tone} style={{ width: `${(segment.value / total) * 100}%` }} title={`${segment.label}: ${segment.value}`} />)}</div> : <p className="mt-3 rounded-xl border border-dashed border-white/[0.08] px-3 py-3 text-center text-xs font-bold text-slate-500">Todavía no hay estados que representar.</p>}
      <dl className="mt-3 grid grid-cols-2 gap-2">
        {segments.map((segment) => <div key={segment.key} className="flex items-center justify-between gap-2 rounded-lg bg-white/[0.035] px-2.5 py-2"><dt className={`text-[10px] font-black uppercase ${segment.text}`}>{segment.label}</dt><dd className="text-xs font-black tabular-nums text-white">{segment.value}</dd></div>)}
      </dl>
    </section>
  );
}
