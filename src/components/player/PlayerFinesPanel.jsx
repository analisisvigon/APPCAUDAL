  ...
  return (
    <div role="status" aria-label="Cargando tus multas" className="space-y-3">
      <div className="grid animate-pulse grid-cols-2 gap-2 sm:grid-cols-4">
        {[0, 1, 2, 3].map((item) => <div key={item} className={`${CARD} h-[82px] bg-white/[0.045]`} />)}
      </div>
      <div className="grid animate-pulse gap-3 lg:grid-cols-2">
        {[0, 1].map((item) => <div key={item} className={`${CARD} h-64 bg-white/[0.045]`} />)}
      </div>
...
function KpiCard({ label, value, tone = 'text-white', detail = null }) {
  return (
    <article className={`${CARD} px-3.5 py-3 sm:px-4`}>
      <p className="text-[8px] font-black uppercase tracking-[0.13em] text-slate-500 sm:text-[9px]">{label}</p>
      <p className={`mt-1.5 truncate text-lg font-black tabular-nums sm:text-xl ${tone}`}>{value}</p>
      {detail ? <p className="mt-1 text-[9px] font-bold text-slate-500">{detail}</p> : null}
    </article>
  );
}
...
function FineBadges({ fine }) {
  ...
  return (
    <div className="flex flex-wrap justify-end gap-1.5">
      ...
      {overdue ? <span className="inline-flex rounded-full border border-red-300/20 bg-red-400/10 px-2 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-red-100">Vencida</span> : null}
    </div>
  );
}
...
function AmountItem({ label, value, tone = 'text-white' }) {
  return (
    <div className="min-w-0">
      <dt className="text-[8px] font-black uppercase tracking-[0.09em] text-slate-500">{label}</dt>
    ...
    <article className={`${CARD} p-4 sm:p-5`}>
      <header className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <time dateTime={fine.occurred_on || undefined} className="mt-1 block text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">{formatFinesDate(fine.occurred_on)}</time>
        </div>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-x-3 gap-y-3 border-y border-white/[0.065] py-3 min-[420px]:grid-cols-3">
        ...
        {surchargeVisible ? <AmountItem label="Total generado" value={fine.generated_amount} /> : null}
        <AmountItem label="Cobrado" value={fine.collected_amount} tone="text-emerald-200" />
        <AmountItem label="Pendiente" value={fine.pending_amount} tone={Number(fine.pending_amount) > 0 ? 'text-amber-100' : 'text-white'} />
        <div className="min-w-0">
          <dt className="text-[8px] font-black uppercase tracking-[0.09em] text-slate-500">Vence</dt>
          <dd className="mt-1 text-sm font-black text-white"><time dateTime={fine.due_on || undefined}>{formatFinesDate(fine.due_on)}</time></dd>
        </div>
      </dl>

      {fine.note ? <section className="mt-3 border-l-2 border-caudal-electric/45 pl-3"><h4 className="text-[9px] font-black uppercase tracking-[0.12em] text-caudal-electric/80">Nota</h4><p className="mt-1 whitespace-pre-wrap break-words text-sm leading-5 text-slate-300">{fine.note}</p></section> : null}
      {fine.lifecycle_status === 'cancelled' && fine.cancellation_reason ? <section className="mt-3 border-l-2 border-slate-500 pl-3"><h4 className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Motivo de anulación</h4><p className="mt-1 whitespace-pre-wrap break-words text-sm leading-5 text-slate-300">{fine.cancellation_reason}</p></section> : null}
    </article>
  );
}
        ...
        summaryState.status === 'error' ? <PlayerFinesError onRetry={retry} /> : (
          <section aria-label="Resumen de mis multas">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <KpiCard label="Multas activas" value={String(summary.active_fines)} />
          ...
          <section aria-labelledby="player-fines-list-title" className="space-y-3">
            <div className={`${CARD} p-3 sm:p-4`}>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="grid grid-cols-3 gap-1 sm:grid-cols-6" aria-label="Filtrar mis multas">
                  {PLAYER_FINE_FILTERS.map((option) => <button key={option.value} type="button" onClick={() => setFilter(option.value)} aria-pressed={filter === option.value} className={`min-h-[38px] rounded-lg px-2 text-[9px] font-black transition ${filter === option.value ? 'bg-caudal-electric text-slate-950' : 'bg-white/[0.055] text-slate-400 hover:bg-white/[0.09] hover:text-white'} ${FOCUS_RING}`}>{option.label}</button>)}
                </div>
              </div>
            </div>

            {listState.rows.length === 0 ? <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.025] px-4 py-9 text-center"><FinesIcon className="mx-auto h-8 w-8 text-slate-600" /><p className="mt-3 text-sm font-black text-slate-200">No tienes multas registradas.</p><p className="mt-1 text-xs text-slate-500">Cuando exista alguna, aparecerá aquí.</p></div> : null}
            {listState.rows.length > 0 && visibleRows.length === 0 ? <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.025] px-4 py-8 text-center"><p className="text-sm font-black text-slate-200">No hay multas cargadas con este estado.</p></div> : null}
