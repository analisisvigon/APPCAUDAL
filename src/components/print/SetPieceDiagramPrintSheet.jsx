...
export default function SetPieceDiagramPrintSheet({ match, title = 'ABP', diagrams = [] }) {
  const printable = diagrams.slice(0, 2);
  return (
    <article className="lineup-print-sheet print-sheet-a4 diagram-print-sheet">
      ...
      <section className="diagram-print-grid">
        {printable.map((diagram, index) => (
          <div key={diagram.id || `${diagram.tipo}-${diagram.orden}-${index}`} className="diagram-print-card">
            ...
            <SetPieceDiagramCanvas elements={diagram.elements || []} readOnly />
