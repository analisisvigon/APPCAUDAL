...
const delegatedEventDefinitions = [
  ...
  { key: 'momento_empujando', tipoEvento: 'momento_empujando', label: 'Empujando arriba', short: 'UP', side: 'caudal', category: 'momentum', tone: 'offensive' },
  { key: 'momento_replegados', tipoEvento: 'momento_replegados', label: 'Replegados', short: 'REP', side: 'caudal', category: 'momentum', tone: 'defensive' },
];
...
function App() {
  ...
  const [delegatedMatchState, setDelegatedMatchState] = useState('EN JUEGO');
  ...
  const resetDelegatedTimer = () => {
    ...
    setDelegatedMatchState('EN JUEGO');
    updateDelegatedMinute(0);
  };

  const pauseDelegatedTimerForHalftime = () => {
    setDelegatedTimerRunning(false);
    setDelegatedMatchState('DESCANSO');
  ...
  const getDelegatedMomentumSegments = () => safeArray(safeObject(selectedMatch?.postAiAnalysis).delegatedMomentumSegments);

  const saveDelegatedMomentumSegment = async (definition) => {
    if (!selectedMatch || !definition) return;
    const minute = Math.max(0, Math.min(130, Number(delegatedMinute) || 0));
    const currentAnalysis = safeObject(selectedMatch.postAiAnalysis);
    const currentSegments = safeArray(currentAnalysis.delegatedMomentumSegments);
    const lastSegment = currentSegments[currentSegments.length - 1];
    const nextSegments = lastSegment && lastSegment.end_minute === null
      ? [
        ...currentSegments.slice(0, -1),
        { ...lastSegment, end_minute: minute },
        ...(lastSegment.state === definition.label ? [] : [{
          id: `${Date.now()}-${definition.key}`,
          start_minute: minute,
          end_minute: null,
    ...
    const nextPostAiAnalysis = { ...currentAnalysis, delegatedMomentumSegments: nextSegments };
    setMatches((current) => current.map((match) => (match.id === selectedMatch.id ? { ...match, postAiAnalysis: nextPostAiAnalysis } : match)));
    const { error } = await supabase.from("partidos").update({ post_ai_analysis: nextPostAiAnalysis }).eq("id", selectedMatch.id);
    if (error) {
      console.error('Error guardando tramo de momento del partido:', { definition, error });
      setStatsError(error.message || 'No se pudo guardar el momento del partido.');
      return;
    }
    setDelegatedEventFeedback(`${definition.label} desde ${minute}'`);
    window.setTimeout(() => setDelegatedEventFeedback((current) => (current === `${definition.label} desde ${minute}'` ? '' : current)), 1400);
  };

  const toggleDelegatedImportantEvent = async (eventId) => {
    if (!selectedMatch || !eventId) return;
    const currentAnalysis = safeObject(selectedMatch.postAiAnalysis);
    const currentIds = safeArray(currentAnalysis.delegatedImportantEventIds);
    const nextIds = currentIds.includes(eventId) ? currentIds.filter((id) => id !== eventId) : [...currentIds, eventId];
    const nextPostAiAnalysis = { ...currentAnalysis, delegatedImportantEventIds: nextIds };
    setMatches((current) => current.map((match) => (match.id === selectedMatch.id ? { ...match, postAiAnalysis: nextPostAiAnalysis } : match)));
    const { error } = await supabase.from("partidos").update({ post_ai_analysis: nextPostAiAnalysis }).eq("id", selectedMatch.id);
    if (error) setStatsError(error.message || 'No se pudo marcar el evento importante.');
  };

  const updateDelegatedEventPlayer = async (eventId, jugadorId) => {
    await updateQuickEvent(eventId, { jugadorId });
    if (jugadorId) {
      setRecentDelegatedPlayerIds((current) => [jugadorId, ...current.filter((id) => id !== jugadorId)].slice(0, 5));
    }
  };

  const undoLastDelegatedEvent = async () => {
    if (!selectedMatch) return;
    const lastEvent = [...getDelegatedEvents()].sort((a, b) => {
      const createdDiff = String(b.createdAt || '').localeCompare(String(a.createdAt || ''));
      return createdDiff || Number(b.minute || 0) - Number(a.minute || 0);
    })[0];
    if (!lastEvent) return;
    setDelegatedEventSaving(true);
  ...
  const renderDelegatedStatsMode = () => {
    ...
    const stateClass = delegatedMatchState === 'FINALIZADO'
      ...
      : delegatedMatchState === 'DESCANSO'
        ? 'border-yellow-200/25 bg-yellow-300/[0.12] text-yellow-100'
        : delegatedTimerRunning
          ...
          <aside className="space-y-4">
            <section className="rounded-3xl border border-white/5 bg-[#091428]/90 p-4 shadow-glow sm:p-5">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <div className="rounded-2xl bg-white/5 p-3 text-center">
                  <p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Tiros / 10'</p>
          ...
          <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/70 px-3 pb-3 pt-10 sm:items-center sm:px-4 sm:py-4">
            <div className="max-h-[86vh] w-full max-w-md overflow-y-auto rounded-t-[2rem] border border-white/10 bg-caudal-950 p-5 shadow-glow sm:rounded-3xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Editar evento</p>
          ...
          <main className="space-y-3 pb-8 sm:space-y-4">
            <section className="relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#0b1220] shadow-[0_16px_48px_rgba(0,0,0,0.24)]">
              ...
              <div className="relative grid gap-4 p-3.5 sm:p-4 lg:grid-cols-[auto_1fr_auto] lg:items-center">
                    ...
                    {isEditingHomePhrase ? (
                      <div className="space-y-2">
                        <textarea
                          ...
                          rows={2}
                          maxLength={180}
                          className="w-full resize-none rounded-2xl border border-white/15 bg-black/25 px-3 py-2 text-sm text-white outline-none transition focus:border-red-300 focus:bg-black/35"
                        />
                        <div className="flex flex-wrap justify-center gap-2 sm:justify-start">
                          <button
                            ...
                            className="rounded-xl bg-red-500/90 px-3 py-2 text-xs font-black text-white transition hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-60"
                          >
                          ...
                          <button
                            ...
                            className="rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold text-slate-100 transition hover:bg-white/15"
                          >
                            Cancelar
                          </button>
                        </div>
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <p className="text-sm font-medium leading-relaxed text-slate-300">{homePhrase}</p>
                        <button
                          type="button"
                          onClick={() => {
                            setHomePhraseDraft(homePhrase);
                          ...
                          className="mx-auto inline-flex shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.06] px-3 py-1.5 text-xs font-semibold text-slate-100 transition hover:bg-white/10 sm:mx-0"
                        >
                          Editar
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-3 gap-2 rounded-[1.15rem] border border-white/10 bg-black/20 p-2 text-center">
                  <div>
                    <p className="text-lg font-black text-white">{homeDashboard.playerCount}</p>
                    <p className="text-[9px] uppercase tracking-[0.14em] text-slate-500">Plantilla</p>
                  </div>
                  <div>
                    <p className="text-lg font-black text-white">{homeDashboard.rivalCount}</p>
                    <p className="text-[9px] uppercase tracking-[0.14em] text-slate-500">Rivales</p>
                  </div>
                  <div>
                    <p className="text-lg font-black text-white">{homeDashboard.balance.pointsPerGame}</p>
            ...
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
              {[
                ...
                <button
                  ...
                  className={`group rounded-[1.35rem] border p-4 text-left transition duration-200 hover:-translate-y-0.5 active:translate-y-0 ${
                    item.tab === 'Partidos'
                      ? 'min-h-32 border-caudal-electric/28 bg-caudal-electric/[0.09] shadow-[0_18px_46px_rgba(79,140,255,0.14)] hover:border-caudal-electric/55 hover:bg-caudal-electric/[0.13] hover:shadow-[0_24px_58px_rgba(79,140,255,0.18)] xl:col-span-2'
                      : `min-h-28 border-white/10 ${item.accent} shadow-[0_12px_35px_rgba(0,0,0,0.16)] hover:border-white/20 hover:bg-white/[0.07]`
                  }`}
                >
                  <span className={`inline-flex ${item.tab === 'Partidos' ? 'text-caudal-electric' : item.iconColor} opacity-90 transition duration-200 group-hover:text-white group-hover:opacity-100`}>
                    {item.icon}
                  </span>
                  <p className={`${item.tab === 'Partidos' ? 'mt-5 text-xl' : 'mt-4 text-base'} font-black text-white`}>{item.title}</p>
            ...
            <section className="grid gap-4 xl:grid-cols-[0.95fr_1.25fr_0.8fr]">
              ...
              <div className="rounded-[1.35rem] border border-white/10 bg-white/[0.045] p-4 shadow-[0_16px_42px_rgba(0,0,0,0.18)]">
                ...
                <div className="mt-4 grid gap-2">
                  {homeDashboard.recent.length ? [...homeDashboard.recent].reverse().map((result) => {
                    const scoreLabel = `${result.score.caudalGoals}-${result.score.rivalGoals}`;
                    return (
                      <div key={`${result.id}-${result.label}`} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border border-white/10 bg-black/15 px-3 py-2.5 transition hover:-translate-y-0.5 hover:bg-white/[0.06]">
                        <span className={`flex h-8 w-8 items-center justify-center rounded-xl text-xs font-black ${
                          result.label === 'V' ? 'bg-emerald-300 text-slate-950' : result.label === 'D' ? 'bg-red-300 text-slate-950' : 'bg-amber-200 text-slate-950'
                        }`}>{result.label}</span>
                        <div className="min-w-0">
                          <p className="truncate text-sm font-black text-white">{scoreLabel} {result.match.opponent}</p>
                          <p className="text-xs text-slate-500">{matchDisplayDate(result.match.date)}</p>
                        </div>
                        <span className="text-xs font-bold uppercase text-slate-500">{result.match.isHome ? 'Local' : 'Visit.'}</span>
                      </div>
                    );
                  }) : <p className="rounded-2xl border border-white/10 bg-white/[0.035] px-3 py-3 text-sm text-slate-400">Sin partidos finalizados.</p>}
                </div>
              </div>
                ...
                {homeDashboard.lastMatch ? (
                  <div className="mt-4">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        openMatchPage(homeDashboard.lastMatch, 'POST');
                        setActiveTab('Partidos');
                      }}
                      className="mt-4 w-full rounded-2xl border border-caudal-electric/25 bg-caudal-electric/10 px-4 py-3 text-sm font-black text-caudal-electric transition duration-200 hover:-translate-y-0.5 hover:bg-caudal-electric hover:text-slate-950"
                    >
                      Ver POST
                    </button>
                  </div>
